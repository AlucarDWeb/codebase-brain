...
def _clean_md(text):
    ...
    text = re.sub(r"(?<!\w)_(?=[^_\s])[^_]{24,}?(?<=[^_\s])_(?!\w)", "", text or "", flags=re.DOTALL)
    lines = []
    ...
    # Emphasis markers only at word boundaries, so snake_case identifiers keep their underscores.
    text = re.sub(r"(?<!\w)([*_]{1,3})(?=\S)([^*_\n]+?)(?<=\S)\1(?!\w)", r"\2", text)
    return text
...
def release_digest(db, tag, web=""):
    ...
    return week_digest(db, start, end, web, rows=rows, label=f"release {tag}", digest_kind="release digest",
                       window_extra={"release": tag})


def week_digest(db, start, end, web="", rows=None, label=None, digest_kind="weekly digest", window_extra=None):
    """The digest data object in the shape the weekly-digest template renders. Every field
    is computed from the commits in the window (or the rows given); nothing is interpreted."""
    m = meta(db)
    ...
    lede_parts = []
    if top_area:
        noun = "release" if digest_kind == "release digest" else "week"
        lede_parts.append(f"Most of the {noun} went into <em>{_html(top_area['name'])}</em> with {len(top_area['prs'])} changes")
    ...
    label = label or f"week of {datetime.date.fromisoformat(start).strftime('%-d %B %Y')}"
    window = {"start": start, "end": end, "label": label, "commits": n}
    window.update(window_extra or {})
    return {"eyebrow": [f"{project}", _fmt_range(start, end), digest_kind],
...
def slice_for_viz(hdb_path, recent=60, churn_limit=40, weeks_limit=26, releases_limit=8):
    ...
    try:
        ...
        return {
            ...
            "release_digests": [d for d in (release_digest(db, r[0], m.get("remote_web", ""))
                                            for r in releases(db, limit=releases_limit)) if d],
